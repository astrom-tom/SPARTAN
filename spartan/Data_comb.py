            ...
            #try:
                if Dat == 0:
                    ...
                    for ll, jj in zip(spec_names, data_dico):
                        ...
                        if float(jj[1]) < 0 :
                            ### if the observed magnitude is negative
                            ### we compute the magnitude from the spectrum itself
                            ...
                            for g in data_dico:
                                if str(g[0])[0] == 'b':
                                    if str(g[0])[2:-1]==gal.Magnitudes_spec[0][0]:
                                        g[1] = gal.Magnitudes_spec[0][1]
                                        g[2] = gal.Magnitudes_spec[0][2]
                                else:
                                    if g[0]==gal.Magnitudes_spec[0][0]:
                                        g[1] = gal.Magnitudes_spec[0][1]
                                        g[2] = gal.Magnitudes_spec[0][2]
                    ...
                    ###save photometry
                    savedata.create_dataset('Mag', data=numpy.array(data_dico).astype(numpy.string_))
